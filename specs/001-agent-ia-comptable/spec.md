...
- **FR-019**: System MUST rafraîchir les vues matérialisées disponibles après chaque synchronisation (6 vues matérialisées + 1 vue de contrôle simple dans le Chantier A, +1 vue `mv_budget_vs_realise` après le Chantier C).
...
- **SC-006** : Les 6 vues matérialisées du Chantier A se rafraîchissent en moins de 60 secondes après synchronisation (via `refresh_all_views()`).
